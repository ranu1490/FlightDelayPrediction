...
def loadData():
    flights = pd.read_csv('/home/ubuntu/flight_delay/vs_flight/flights.csv')
    airport = pd.read_csv('/home/ubuntu/flight_delay/vs_flight/airports.csv')
    ...
    if choice=="Random Forest Regressor":
        ...
        if st.button("Predict using Random Forest Regressor"):
            ...
            if(res>=0):
                ...
                text2= "Flight is only delayed by "+str(abs(res))+" minutes. Delays upto 15 minutes are considered as not delay.\n FLIGHT IS NOT DELAYED"
                st.write(text2)
            else:
                text3= "Flight is delayed by "+str(res)+" minutes. Delays by more than 15 minutes are considered to be actual delays.\n FLIGHT IS DELAY"
                st.write(text3)
